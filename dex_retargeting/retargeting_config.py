...
class RetargetingConfig:
    ...
    def __post_init__(self):
        ...
        if self.type == "vector":
            ...
            if self.target_origin_link_names is None or self.target_task_link_names is None:
                raise ValueError(f"Custom retargeting requires: target_origin_link_names + target_task_link_names")
            if len(self.target_task_link_names) != len(self.target_origin_link_names):
                raise ValueError(f"Custom retargeting origin and task links dim mismatch")
            if self.target_link_human_indices.shape != (2, len(self.target_origin_link_names)):
                raise ValueError(f"Custom retargeting link names and link indices dim mismatch")
            if self.target_link_human_indices is None:
                raise ValueError(f"Custom retargeting requires: target_link_human_indices")
