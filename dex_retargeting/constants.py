...
class RetargetingType(enum.Enum):
    ...
    custom = enum.auto()  # For teleoperation, with finger closing prior
...
def get_config_path(robot_name: RobotName, retargeting_type: RetargetingType, hand_type: HandType) -> Path:
    ...
    elif retargeting_type == RetargetingType.custom:
        config_name = f"{robot_name_str}_{hand_type_str}_custom.yml"
    else:
