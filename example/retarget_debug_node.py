...
class constructor():
    ...
    def __init__(self,  retargeting: SeqRetargeting, output_path: str, config_path: str):
        ...
        self.idx = np.array( [5904, 5863, 5596, 6013] )
        self.seq_num = self.idx[3]
        ...
        bagIn = rosbag.Bag( "/home/xujiahe/retarget_debug.bag", "r")
        self.cam1_joint_msg = None
        self.cam2_joint_msg = None
        for topic, msg, t in bagIn.read_messages(topics=["/cam1/joint_pos"]):
            if(msg.header.seq == self.seq_num):
                self.cam1_joint_msg = msg
                break

        for topic, msg, t in bagIn.read_messages(topics=["/cam2/joint_pos"]):
            if(msg.header.seq == self.seq_num+23):
                self.cam2_joint_msg = msg
                break
    ...
    #def callback(self, cam1_joint_msg, cam2_joint_msg):
        #bagIn = rosbag.Bag( "/home/xujiahe/retarget_debug.bag", "r")
        cam1_joint_msg = self.cam1_joint_msg
        cam2_joint_msg = self.cam2_joint_msg
        # for topic, msg, t in bagIn.read_messages(topics=["/cam1/joint_pos"]):
